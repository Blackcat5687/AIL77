                ...
                if (existing != null) {
                    dao.update(
                        existing.copy(
                            ...
                            updatedAt = System.currentTimeMillis(), pinned = isPinned,
                            archived = isArchived, backgroundImageUri = backgroundImageUri
                        )
                    )
                } else if (noteId == -1L) {
                    dao.insert(
                        Note(
                            title = title, content = content, color = selectedColor,
                            pinned = isPinned, archived = isArchived, backgroundImageUri = backgroundImageUri
                        )
                    )
                }
            }
